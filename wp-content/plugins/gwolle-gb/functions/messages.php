...
function gwolle_gb_get_messages() {
	...
	if ( is_array( $gwolle_gb_error_fields ) && !empty( $gwolle_gb_error_fields ) ) {
		...
		$messages .= '<p class="error_fields"><strong>' . __('There were errors submitting your guestbook entry.', 'gwolle-gb') . '</strong></p>';
		$messages .= '<p class="error_fields" style="display: none;">' . print_r( $gwolle_gb_error_fields, true ) . '</p>';
	}

	$_gwolle_gb_messages = apply_filters( 'gwolle_gb_messages', $gwolle_gb_messages );

	foreach ( $_gwolle_gb_messages as $message ) {
		$messages .= $message;
	}
